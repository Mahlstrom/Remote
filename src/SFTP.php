...
class SFTP extends RemoteHelper implements remoteInterface {
	...
	public function get($local_file, $remote_file, $offset = 0) {
		...
		$this->_checkSoLocalDirExists($local_file);
	...
	public function readDir($dir = '.') {
		...
		foreach($data as $filename => $fileData) {
			$ret[$filename] = $this->_createRemoteFile($filename, $fileData);
		}
		return $ret;
	}
	...
	public function stat($filename) {
		...
		return $this->_createRemoteFile($filename, $fileData);
	}
	 ...
	 * @return RemoteFile
	 */
	private function _createRemoteFile($filename, $fileData) {
